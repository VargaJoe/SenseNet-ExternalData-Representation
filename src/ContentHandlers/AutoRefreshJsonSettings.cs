...
{
    [ContentHandler]
    public class AutoRefreshJsonSettings : Settings //DynamicJsonContent
    {
        public AutoRefreshJsonSettings(Node parent) : this(parent, null) { }
        public AutoRefreshJsonSettings(Node parent, string nodeTypeName) : base(parent, nodeTypeName) { }
        protected AutoRefreshJsonSettings(NodeToken nt) : base(nt) { }
        ...
        // should be called ExternalJsonSettings
        ...
        private bool IsRefreshTime
        {
            ...
            {
                bool result = false;
                //Technical Debt: explain in details, when and what we check      
        ...
        private void SaveAsTechnicalUser()
        {
            ...
            try
            {
                ...
                using (new SystemAccount())
                {
                    ...
                    //var nodeToSave = this;
                    var nodeToSave = Node.Load<AutoRefreshJsonSettings>(this.Id);
                    while (!ok && count > 0)
                    {
                        try
                        {
                            ...
                            {
                                nodeToSave = Node.Load<AutoRefreshJsonSettings>(this.Id);
